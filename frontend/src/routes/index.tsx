...
export default component$(() => {
	...
	const handleShortenerClick$ = $(async () => {
		...
		if (!RegExp(/(http(s?)):\/\//i).test(url)) {
			url = `https://${url}`;
		}

		const { newUrl } = await getShortenUrl$(url);
		...
		<>
			<div class="container">
				<div className="d-flex flex-row-reverse my-5">
					<a
						href="https://github.com/origranot/url-shortener"
						className="github-button"
						data-size="large"
						data-show-count="true"
						aria-label="Star origranot/url-shortener on Github"
					>
						{" "}
						Star
					</a>
			...
			<div class="container">
				<h1 class="p-3 text-light font-weight-bold">URL Shortener</h1>
				<div class="alert alert-primary" role="alert">
					Add your very long <b>URL</b> in the input bellow and click
					on the button to make it shorter
				</div>
				<div class="input-group mb-3">
					<input
						...
						onKeyPress$={(event) => handleShortenerKeypress$(event)}
						aria-label="url"
