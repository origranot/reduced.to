...
describe('AnalyticsService.getClicksOverTime() getClicksOverTime method', () => {
  ...
  describe('Happy Path', () => {
    it('should return clicks over time for the given linkId and durationDays', async () => {
      ...
      expect(mockPrismaService.$queryRaw).toHaveBeenCalledWith(expect.any(Array), 'day', linkId, expect.any(Date));
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle a duration of 1 day correctly', async () => {
      ...
      // Assert
      expect(result).toEqual(expectedResult);
      expect(mockPrismaService.$queryRaw).toHaveBeenCalledWith(expect.any(Array), 'hour', linkId, expect.any(Date));
    });
