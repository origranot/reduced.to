...
describe('AnalyticsController.getBrowsers() getBrowsers method', () => {
  ...
  describe('Happy Path', () => {
    it('should return grouped browser data for a valid link', async () => {
      // Arrange
      const key = 'valid-key';
      const days = 7;
      const link = { id: 'link-id', url: 'http://example.com' };
      const groupedData = [{ browser: 'Chrome', count: 10 }];
      ...
      expect(mockAnalyticsService.getGroupedByField).toHaveBeenCalledWith(link.id, 'browser', days, undefined);
    });
  });
