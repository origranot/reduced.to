...
describe('AnalyticsController.getOs() getOs method', () => {
  ...
  describe('Happy Path', () => {
    it('should return grouped data by OS', async () => {
      // Arrange
      const key = 'test-key';
      const days = 7;
      const link = { id: 'link-id', url: 'http://example.com' };
      const groupedData = [{ os: 'Windows', count: 10 }];
      ...
      // Act
      const result = await analyticsController.getOs(key, days, mockUserContext as any);
      ...
      expect(mockAnalyticsService.getGroupedByField).toHaveBeenCalledWith(link.id, 'os', days, undefined);
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should throw NotFoundException if link is not found', async () => {
      ...
      // Act & Assert
      await expect(analyticsController.getOs(key, days, mockUserContext as any)).rejects.toThrow(NotFoundException);
      expect(mockPrismaService.link.findUnique).toHaveBeenCalledWith({
        where: { key, userId: mockUserContext.id },
        select: { id: true, url: true },
      });
